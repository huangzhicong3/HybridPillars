...
from . import pointnet2_batch_cuda as cuda_ops
...
class PointnetSAModuleMSG_pillar(_PointnetSAModuleBase):
    """Pointnet set abstraction layer with multiscale grouping"""
    ...
    def __init__(self, *, npoint: int, radii: List[float], nsamples: List[int], mlps: List[List[int]], bn: bool = True,
        ...
        for i in range(len(radii)):
            ...
            nsample = nsamples[i]
            self.groupers.append(
                pointnet2_utils.PillarQueryAndGroup(radius, nsample, use_xyz=use_xyz)
                if npoint is not None else pointnet2_utils.GroupAll(use_xyz)
            )
            mlp_spec = mlps[i]
            if use_xyz:
...
class PointnetSAModuleMSG_WithSampling(_PointnetSAModuleBase):
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None, cls_features: torch.Tensor = None, new_xyz=None,
                ctr_xyz=None, export=None, new_feature_ball_query=None):
        """
        :param xyz: (B, N, 3) tensor of the xyz coordinates of the features
        ...
        new_features_list = []
        new_feature_ball_query_list = []
        sampled_idx_list = []

        if export == 1:
            for i in range(3):
                new_features = new_feature_ball_query[i]
                new_features = self.mlps[i](new_features)
                if self.pool_method == 'max_pool':
                    new_features = F.max_pool2d(
                        new_features, kernel_size=[1, self.nsamples[i]]
                    )  # (B, mlp[-1], npoint, 1)
                elif self.pool_method == 'avg_pool':
                    new_features = F.avg_pool2d(
                        new_features, kernel_size=[1, self.nsamples[i]]
        ...
        if len(self.groupers) > 0:
            for i in range(len(self.groupers)):
                new_features = self.groupers[i](xyz, new_xyz, features)  # (B, C, npoint, nsample)
                if export == 0:
                    new_feature_ball_query_list.append(new_features.clone())
                new_features = self.mlps[i](new_features)  # (B, mlp[-1], npoint, nsample)
                if self.pool_method == 'max_pool':
                    new_features = F.max_pool2d(
                        new_features, kernel_size=[1, new_features.size(3)]
        ...
        if export == 0:
            return new_xyz, new_features, cls_features, new_feature_ball_query_list
...
class PointnetSAModuleMSG_WithSampling_pillar(_PointnetSAModuleBase):
    """Pointnet set abstraction layer with specific downsampling and multiscale grouping """
    ...
    def __init__(self, *,
        ...
        for i in range(len(radii)):
            ...
            if self.dilated_group:
                ...
                self.groupers.append(
                    pointnet2_utils.PillarQueryAndGroup(
                        radius, nsample, use_xyz=use_xyz)
                    if npoint_list is not None else pointnet2_utils.GroupAll(use_xyz)
                )
            mlp_spec = mlps[i]
...
class PointnetSAModule_pillar(PointnetSAModuleMSG_pillar):
    """Pointnet set abstraction layer"""
