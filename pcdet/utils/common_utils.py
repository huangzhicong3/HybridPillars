...
def _torch_gather(x: torch.tensor, ind: torch.tensor, ind_bid: torch.tensor = None,
                  channel_first: bool = False, c=0) -> torch.tensor:
    assert x.is_contiguous()
    assert ind.is_contiguous()
    if ind_bid is None:  # batch gather
        (b, *s) = ind.shape
        if channel_first:
            ret = x.gather(-1, ind.view(b, 1, -1).expand(-1, c, -1)).view(b, -1, *s)
        else:
            ret = x.gather(1, ind.view(b, -1, 1).expand(-1, -1, c)).view(b, *s, -1)
...
def torch_gather(x: torch.tensor, ind: torch.tensor, ind_bid: torch.tensor = None,
    ...
    if torch.onnx.is_in_onnx_export():
        ...
        return _torch_gather(x, ind, ind_bid, channel_first, c=c)
    else:
        return _torch_gather(x, ind, ind_bid, channel_first, c=c)
